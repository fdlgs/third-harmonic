...
pub fn run(verbose: bool) -> Result<()> {
    ...
    let StateEvolution { time_ts, .. } = evolution;
    println!(
        "StateEvolution::new(alpha_square={ALPHA_SQUARE}) over {} ts finished on {} cores in {:?}",
        time_ts.len(),
...
/// ### Errors
/// 
/// - saving the plot may fail with `ruziv::core::error::PlottingError`
...
/// - `verbose` - verbosity
#[must_use]
pub fn poisson_distribution(alpha_square: u32, verbose: bool) -> Array1<f64> {
...
impl StateEvolution {
    ...
    pub fn new(alpha_square: u32, delta_t: f64, step_cnt: u16, pb_th: f64, verbose: bool) -> Result<Self> {
        ...
        if verbose {
            println!("unused chunk_size {}, time_ts {time_ts}", chunk_size(step_cnt));
        }
